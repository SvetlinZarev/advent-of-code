...
first three unknown springs must be broken, then operational, then
...
afterward would have been `2,3` instead.) Since each `??` can either be `#.`
...
were #, the first number would have to be 4 or higher). However, the remaining
run of unknown spring conditions have many different ways they could hold groups
