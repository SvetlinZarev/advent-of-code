use aoc_shared::input::{load_line_delimited_input_from_file, load_text_input_from_file};
use aoc_shared::parsing::parse_line_delimited;
use criterion::{criterion_group, criterion_main, Criterion};

use aoc_day_04::{part_one, part_two_v1, part_two_v2, part_two_v3, part_two_v4, Card};

criterion_group!(
    benches,
    benchmark_parsing,
    benchmark_part_one,
    benchmark_part_two
);
criterion_main!(benches);

fn benchmark_parsing(c: &mut Criterion) {
    let input = load_text_input_from_file("inputs/input.txt");

    c.bench_function("parsing", |b| {
        b.iter(|| parse_line_delimited::<_, Card>(&input));
    });
}

fn benchmark_part_one(c: &mut Criterion) {
    let input = load_line_delimited_input_from_file("inputs/input.txt");

    c.bench_function("part-1", |b| {
        b.iter(|| part_one(&input));
    });
}

fn benchmark_part_two(c: &mut Criterion) {
    let input = load_line_delimited_input_from_file("inputs/input.txt");

    c.bench_function("part-2/v1", |b| {
        b.iter(|| part_two_v1(&input));
    });

    c.bench_function("part-2/v2", |b| {
        b.iter(|| part_two_v2(&input));
    });

    c.bench_function("part-2/v3", |b| {
        b.iter(|| part_two_v3(&input));
    });

    c.bench_function("part-2/v4", |b| {
        b.iter(|| part_two_v4(&input));
    });
}
