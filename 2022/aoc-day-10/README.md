...
* During the 20th cycle, register `X` has the value `21`, so the signal strength
  is `20 * 21 = 420`. (The 20th cycle occurs in the middle of the
  second `addx -1`, so the value of register `X` is the starting value, `1`,
  plus all of the other `addx` values up to that
...
a `#` (`@` - drawn),here are the cycles during which the first and last pixel in
each row are drawn:
