...
either `on` or `off`; at the start of the reboot process, they are all off. (
...
cuboid (the set of all cubes that have coordinates which fall within ranges
for `x`, `y`, and `z`) and whether to turn all of the cubes in that cuboid `on`
or `off`.
...
positions of at least `-50` and at most `50`. For now, ignore cubes outside this
region.
