...
pub fn load_line_delimited_input_autodetect<O: FromStr<Err=impl Debug>>() -> Vec<O> {
    parse_line_delimited(load_input_autodetect())
}
...
pub fn file_line_delimited<O: FromStr<Err=impl Debug>, P: AsRef<Path>>(path: P) -> Vec<O> {
...
pub fn load_input_from_file<P: AsRef<Path>>(path: P) -> String {
